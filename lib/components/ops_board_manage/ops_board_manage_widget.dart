...
import 'ops_board_manage_model.dart';
export 'ops_board_manage_model.dart';

class OpsBoardManageWidget extends StatefulWidget {
  const OpsBoardManageWidget({
    super.key,
  ...
  final int? opsID;
  final bool? newBoard;
  final String? tenantPrompt;
  final Future Function(int? boardID)? actionNew;
  final String? orgPrompt;

  @override
  State<OpsBoardManageWidget> createState() => _OpsBoardManageWidgetState();
}

class _OpsBoardManageWidgetState extends State<OpsBoardManageWidget>
    with TickerProviderStateMixin {
  late OpsBoardManageModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => OpsBoardManageModel());

    // On component load action.
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      _model.tenantPrompt = FFAppState().session.tenantPrompt;
      safeSetState(() {});
      if (!widget.newBoard!) {
        _model.opsBoardToEdit = await OpsBoardTable().queryRows(
        ...
        safeSetState(() {
          _model.nameTextFieldTextController?.text =
              _model.opsBoardToEdit!.firstOrNull!.name;
        });
        safeSetState(() {
          _model.descriptionTextFieldTextController?.text =
              _model.opsBoardToEdit!.firstOrNull!.description!;
        });
        safeSetState(() {
          _model.promptTextFieldTextController?.text =
              _model.opsBoardToEdit!.firstOrNull!.prompt!;
        });
      }
    });
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        if (widget.newBoard ?? true)
                          FFButtonWidget(
                            onPressed: () async {
                              _model.newTenantRecord =
                                  await OpsBoardTable().insert({
